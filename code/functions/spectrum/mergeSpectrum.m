function [sp3, sampling3] = mergeSpectrum(sp1, sampling1, sp2, sampling2)
% MERGESPECTRUM Merge two spectra whose values are defined at different sampling points
...
% Sort by lower wavelength boundary
if ~issorted(sp1(1,:))
    warning('mergeSpectrum:unsortedInput', ...
        'The merged spectrum will be sorted to increasing wavelength beacause the first spectrum is unsorted.');
    [~, ix1] = sort(sp1(1,:));
    sp1 = sp1(:, ix1);
end
if ~issorted(sp2(1,:))
    warning('mergeSpectrum:unsortedInput', ...
        'The merged spectrum will be sorted to increasing wavelength beacause the second spectrum is unsorted.');
    [~, ix2] = sort(sp2(1,:));
    sp2 = sp2(:, ix2);
end
