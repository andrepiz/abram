...
muPixel = rend.camera.muPixel;
d_body2cam = rend.scene.d_body2cam;
d_body2light = rend.scene.d_body2light;
Rlight = rend.light.radius;
Apupil = rend.camera.Apupil;
f = rend.camera.f;
Rbody = rend.body.radius;
pNorm = rend.body.pNorm;
L_bw = rend.light.L.values;

%% Power computed by ABRAM
PL_pixel = rend.matrix.values.*rend.matrix.adim;
P_bw = PL_pixel.*L_bw;
P_bw(P_bw == 0) = nan;
...
P_bw_comp = pNorm * Apx * cos(alpha_bs).^4.*Apupil/(f^2)...
    .*cos(delta_i).*(Rlight/d_body2light)^2.*L_bw;
